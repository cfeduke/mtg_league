...
Using PostgreSQL as the RDBMS because Postgres is great and if need to deploy to Heroku I can. The database.yml uses passwordless authentication which can be setup by editing `$POSTGRES_HOME/pg_hba.conf` (default filename) and adding a line like:
